...
This function should only modify configuration layer settings."
  (setq-default
   ...
   dotspacemacs-additional-packages '(key-combo wdired dired-subtree gradle-mode groovy-mode)
...
before packages are loaded."
  ...
  ;; Share clipboard
  ;;
  (defun darwin-clipboard-p ()
    (and (eq system-type 'darwin)
         (executable-find "pbcopy")
         (executable-find "pbpaste")))

  (defun darwin-clipboard-paste ()
    (shell-command-to-string "pbpaste"))

  (defun darwin-clipboard-cut (text &optional push)
    (let ((process-connection-type nil))
      (let ((proc (start-process "pbcopy" "*Messages*" "pbcopy")))
        (process-send-string proc text)
        (process-send-eof proc))))

  (defun linux-clipboard-p()
    (and (eq system-type 'gnu/linux)
         (executable-find "xsel")))

  (defun linux-clipboard-paste ()
    (shell-command-to-string "xsel -b -o"))

  (defun linux-clipboard-cut (text &optional push)
    (let ((process-connection-type nil))
  ...
  ;; Parents color
  ;;
  (defun highlights-parentheses-using-stronger-colors ()
    (interactive)
    (cl-loop
  ...
  (add-hook 'emacs-startup-hook
            'highlights-parentheses-using-stronger-colors)
