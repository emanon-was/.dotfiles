...
if [ -z "${DOTFILES_HOME:-}" ]; then
  ...
  dotfiles_dist_home="$(cd "$dotfiles_bin_dir/.." 2>/dev/null && pwd -P || true)"
  if [ -n "$dotfiles_dist_home" ] && [ -d "$dotfiles_dist_home/project-templates" ]; then
    DOTFILES_HOME="$dotfiles_dist_home"
  else
