...
- install 時の symlink と backup は `$HOME/.local/state/dotfiles/init-manifest.tsv` に記録する。
...
- `dist/uninstall.sh` は init manifest を参照して管理対象 symlink を削除し、install 時に退避した backup を元の名前へ復元する。
