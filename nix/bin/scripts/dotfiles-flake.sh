...
Description:
  ...
  build   dotfiles-profile package を build し、profile/ を再生成します。
...
flake_build() {
  ...
  profile="$HOME_MANAGER_ROOT/profile"
  tmp_profile="$HOME_MANAGER_ROOT/.profile.tmp.$$"
  out="$(nix build "$HOME_MANAGER_ROOT#dotfiles-profile" --no-link --print-out-paths)"
