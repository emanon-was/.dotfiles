...
`nix/bin/symsync` の現在仕様をまとめます。

## 方針

- この package は `symsync` binary package として完結する。
- `dotfiles` dispatcher package とは別 package とし、相互依存させない。
- profile 展開で利用する symlink tree synchronization の挙動は、この package の Go test で検査する。
- profile 成果物側のテストでは `symsync apply/unapply` の詳細挙動を検査しない。
...
- `make check` は `nix/bin/symsync/src` で `go test ./...` を実行する。
- profile 構造テストでは、`symsync` binary と completion が含まれることだけを確認する。
