...
dispatcher は repository root または local root を検出し、子 command に環境変数を渡します。

`DOTFILES_HOME` には検出した root が入ります。

repository root は `flake.nix`、`home.nix`、`nix/` がある directory、local root は `.local/bin` と `.local/share/dotfiles` がある directory です。外部から `DOTFILES_HOME` を指定した場合も同じ条件で検証します。
...
- dispatcher は repository root または local root を検出し、子 command に environment を渡します。
